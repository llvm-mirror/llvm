...
/// EVMOperand - Instances of this class represent a parsed machine
/// instruction
struct EVMOperand : public MCParsedAsmOperand {
  ...
  EVMOperand(KindTy K) : MCParsedAsmOperand(), Kind(K) {}
...
public:
  ...
  void addFenceArgOperands(MCInst &Inst, unsigned N) const {
    ...
    for (char c : SE->getSymbol().getName()) {
      ...
      case 'i': Imm |= EVMFenceField::I; break;
      case 'o': Imm |= EVMFenceField::O; break;
      case 'r': Imm |= EVMFenceField::R; break;
      case 'w': Imm |= EVMFenceField::W; break;
      }
    }
    Inst.addOperand(MCOperand::createImm(Imm));
  }
  ...
  // Returns the rounding mode represented by this EVMOperand. Should only
  // be called after checking isFRMArg.
  EVMFPRndMode::RoundingMode getRoundingMode() const {
    // isFRMArg has validated the operand, meaning this cast is safe.
    auto SE = cast<MCSymbolRefExpr>(getImm());
    EVMFPRndMode::RoundingMode FRM =
        EVMFPRndMode::stringToRoundingMode(SE->getSymbol().getName());
    assert(FRM != EVMFPRndMode::Invalid && "Invalid rounding mode");
    return FRM;
  }
...
unsigned EVMAsmParser::validateTargetOperandClass(MCParsedAsmOperand &AsmOp,
  ...
  bool IsRegFPR32 =
      EVMMCRegisterClasses[EVM::FPR32RegClassID].contains(Reg);
  bool IsRegFPR32C =
      EVMMCRegisterClasses[EVM::FPR32CRegClassID].contains(Reg);
...
bool EVMAsmParser::generateImmOutOfRangeError(
    OperandVector &Operands, uint64_t ErrorInfo, int64_t Lower, int64_t Upper,
    Twine Msg = "immediate must be an integer in the range") {
  SMLoc ErrorLoc = ((EVMOperand &)*Operands[ErrorInfo]).getStartLoc();
...
bool EVMAsmParser::MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((EVMOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IDLoc;
    }
...
OperandMatchResultTy EVMAsmParser::parseRegister(OperandVector &Operands,
                                                   bool AllowParens) {
  SMLoc FirstS = getLoc();
  ...
  case AsmToken::Identifier:
    ...
    if (HadParens)
      Operands.push_back(EVMOperand::createToken("(", FirstS, isRV64()));
    SMLoc S = getLoc();
    SMLoc E = SMLoc::getFromPointer(S.getPointer() - 1);
    getLexer().Lex();
    Operands.push_back(EVMOperand::createReg(RegNo, S, E, isRV64()));
  }

  if (HadParens) {
    getParser().Lex(); // Eat ')'
    Operands.push_back(EVMOperand::createToken(")", getLoc(), isRV64()));
  }
...
EVMAsmParser::parseCSRSystemRegister(OperandVector &Operands) {
  SMLoc S = getLoc();
  const MCExpr *Res;
  ...
  case AsmToken::String: {
    ...
    if (CE) {
      ...
      if (isUInt<12>(Imm)) {
        ...
        Operands.push_back(EVMOperand::createSysReg(
  ...
  case AsmToken::Identifier: {
    ...
    auto SysReg = EVMSysReg::lookupSysRegByName(Identifier);
...
OperandMatchResultTy EVMAsmParser::parseImmediate(OperandVector &Operands) {
  SMLoc S = getLoc();
  SMLoc E = SMLoc::getFromPointer(S.getPointer() - 1);
  const MCExpr *Res;
  ...
  Operands.push_back(EVMOperand::createImm(Res, S, E, isRV64()));
  return MatchOperand_Success;
}

OperandMatchResultTy
EVMAsmParser::parseOperandWithModifier(OperandVector &Operands) {
  SMLoc S = getLoc();
  SMLoc E = SMLoc::getFromPointer(S.getPointer() - 1);
  ...
  EVMMCExpr::VariantKind VK = EVMMCExpr::getVariantKindForName(Identifier);
  if (VK == EVMMCExpr::VK_EVM_Invalid) {
  ...
  const MCExpr *ModExpr = EVMMCExpr::create(SubExpr, VK, getContext());
  Operands.push_back(EVMOperand::createImm(ModExpr, S, E, isRV64()));
  return MatchOperand_Success;
}

OperandMatchResultTy EVMAsmParser::parseBareSymbol(OperandVector &Operands) {
  SMLoc S = getLoc();
  SMLoc E = SMLoc::getFromPointer(S.getPointer() - 1);
  const MCExpr *Res;
...
OperandMatchResultTy EVMAsmParser::parseJALOffset(OperandVector &Operands) {
...
OperandMatchResultTy
EVMAsmParser::parseMemOpBaseReg(OperandVector &Operands) {
...
bool EVMAsmParser::ParseInstruction(ParseInstructionInfo &Info,
  ...
  // for the entire file.
  if (getSTI().getFeatureBits()[EVM::FeatureRelax]) {
    auto *Assembler = getTargetStreamer().getStreamer().getAssemblerPtr();
    if (Assembler != nullptr) {
      EVMAsmBackend &MAB =
          static_cast<EVMAsmBackend &>(Assembler->getBackend());
      MAB.setForceRelocs();
    }
  }

  // First operand is token for instruction
  Operands.push_back(EVMOperand::createToken(Name, NameLoc, isRV64()));
...
bool EVMAsmParser::classifySymbolRef(const MCExpr *Expr,
                                       EVMMCExpr::VariantKind &Kind,
                                       int64_t &Addend) {
  Kind = EVMMCExpr::VK_EVM_None;
  Addend = 0;

  if (const EVMMCExpr *RE = dyn_cast<EVMMCExpr>(Expr)) {
    Kind = RE->getKind();
    Expr = RE->getSubExpr();
  }
  ...
  return Kind != EVMMCExpr::VK_EVM_Invalid;
}
...
bool EVMAsmParser::parseDirectiveOption() {
  ...
  if (Option == "rvc") {
    ...
    setFeatureBits(EVM::FeatureStdExtC, "c");
    return false;
  }
  ...
  if (Option == "norvc") {
    ...
    clearFeatureBits(EVM::FeatureStdExtC, "c");
    return false;
  }
  ...
  if (Option == "relax") {
    ...
    setFeatureBits(EVM::FeatureRelax, "relax");
    return false;
  }
  ...
  if (Option == "norelax") {
    ...
    clearFeatureBits(EVM::FeatureRelax, "relax");
    return false;
  }
...
void EVMAsmParser::emitLoadImm(unsigned DestReg, int64_t Value,
                                 MCStreamer &Out) {
  EVMMatInt::InstSeq Seq;
  EVMMatInt::generateInstSeq(Value, isRV64(), Seq);

  unsigned SrcReg = EVM::X0;
  for (EVMMatInt::Inst &Inst : Seq) {
    if (Inst.Opc == EVM::LUI) {
      emitToStreamer(
          Out, MCInstBuilder(EVM::LUI).addReg(DestReg).addImm(Inst.Imm));
    } else {
      emitToStreamer(
...
void EVMAsmParser::emitLoadLocalAddress(MCInst &Inst, SMLoc IDLoc,
                                          MCStreamer &Out) {
  ...
  const EVMMCExpr *Symbol = EVMMCExpr::create(
      Inst.getOperand(1).getExpr(), EVMMCExpr::VK_EVM_PCREL_HI, Ctx);

  emitToStreamer(
      Out, MCInstBuilder(EVM::AUIPC).addOperand(DestReg).addExpr(Symbol));

  const MCExpr *RefToLinkTmpLabel =
      EVMMCExpr::create(MCSymbolRefExpr::create(TmpLabel, Ctx),
                          EVMMCExpr::VK_EVM_PCREL_LO, Ctx);

  emitToStreamer(Out, MCInstBuilder(EVM::ADDI)
                          .addOperand(DestReg)
                          .addOperand(DestReg)
...
bool EVMAsmParser::processInstruction(MCInst &Inst, SMLoc IDLoc,
  ...
  if (Inst.getOpcode() == EVM::PseudoLI) {
    ...
    return false;
  } else if (Inst.getOpcode() == EVM::PseudoLLA) {
