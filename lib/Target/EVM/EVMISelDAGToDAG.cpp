...
static SDNode *selectImm(SelectionDAG *CurDAG, const SDLoc &DL, int64_t Imm,
  ...
  EVMMatInt::generateInstSeq(Imm, XLenVT == MVT::i64, Seq);

  SDNode *Result;
  SDValue SrcReg = CurDAG->getRegister(EVM::X0, XLenVT);
  for (EVMMatInt::Inst &Inst : Seq) {
    SDValue SDImm = CurDAG->getTargetConstant(Inst.Imm, DL, XLenVT);
    if (Inst.Opc == EVM::LUI)
      Result = CurDAG->getMachineNode(EVM::LUI, DL, XLenVT, SDImm);
    else
...
void EVMDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    ReplaceNode(Node, CurDAG->getMachineNode(EVM::ADDI, DL, VT, TFI, Imm));
    return;
  }
  case ISD::SRL: {
    ...
    if (Op1.getOpcode() == ISD::Constant &&
      ...
      if ((Mask | maskTrailingOnes<uint64_t>(ShAmt)) == 0xffffffff) {
        ...
        CurDAG->SelectNodeTo(Node, EVM::SRLIW, XLenVT, Op0.getOperand(0),
                             ShAmtVal);
        return;
      }
    }
...
bool EVMDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
bool EVMDAGToDAGISel::SelectAddrFI(SDValue Addr, SDValue &Base) {
...
void EVMDAGToDAGISel::doPeepholeLoadStoreADDI() {
  ...
  while (Position != CurDAG->allnodes_begin()) {
    ...
    if (!Base.isMachineOpcode() || Base.getMachineOpcode() != EVM::ADDI)
      continue;
