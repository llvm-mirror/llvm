//===-- EVMExpandPseudoInsts.cpp - Expand pseudo instructions -----------===//
//
...
#define EVM_EXPAND_PSEUDO_NAME "EVM pseudo instruction expansion pass"

namespace {

class EVMExpandPseudo : public MachineFunctionPass {
public:
  const EVMInstrInfo *TII;
  static char ID;

  EVMExpandPseudo() : MachineFunctionPass(ID) {
    initializeEVMExpandPseudoPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override { return EVM_EXPAND_PSEUDO_NAME; }
...
char EVMExpandPseudo::ID = 0;

bool EVMExpandPseudo::runOnMachineFunction(MachineFunction &MF) {
  TII = static_cast<const EVMInstrInfo *>(MF.getSubtarget().getInstrInfo());
  bool Modified = false;
  for (auto &MBB : MF)
...
bool EVMExpandPseudo::expandMBB(MachineBasicBlock &MBB) {
  bool Modified = false;
...
static void doAtomicBinOpExpansion(const EVMInstrInfo *TII, MachineInstr &MI,
  ...
  case AtomicRMWInst::Nand:
    BuildMI(LoopMBB, DL, TII->get(EVM::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    BuildMI(LoopMBB, DL, TII->get(EVM::XORI), ScratchReg)
        .addReg(ScratchReg)
        .addImm(-1);
    break;
  }
  BuildMI(LoopMBB, DL, TII->get(getSCForRMW(Ordering, Width)), ScratchReg)
      .addReg(AddrReg)
      .addReg(ScratchReg);
  BuildMI(LoopMBB, DL, TII->get(EVM::BNE))
      .addReg(ScratchReg)
      .addReg(EVM::X0)
      .addMBB(LoopMBB);
}

static void insertMaskedMerge(const EVMInstrInfo *TII, DebugLoc DL,
  ...
  BuildMI(MBB, DL, TII->get(EVM::XOR), ScratchReg)
      .addReg(OldValReg)
      .addReg(NewValReg);
  BuildMI(MBB, DL, TII->get(EVM::AND), ScratchReg)
      .addReg(ScratchReg)
      .addReg(MaskReg);
  BuildMI(MBB, DL, TII->get(EVM::XOR), DestReg)
      .addReg(OldValReg)
      .addReg(ScratchReg);
}

static void doMaskedAtomicBinOpExpansion(
    const EVMInstrInfo *TII, MachineInstr &MI, DebugLoc DL,
  ...
  case AtomicRMWInst::Xchg:
    BuildMI(LoopMBB, DL, TII->get(EVM::ADD), ScratchReg)
        .addReg(EVM::X0)
        .addReg(IncrReg);
    break;
  ...
  case AtomicRMWInst::Sub:
    BuildMI(LoopMBB, DL, TII->get(EVM::SUB), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Nand:
...
bool EVMExpandPseudo::expandAtomicBinOp(
...
static void insertSext(const EVMInstrInfo *TII, DebugLoc DL,
                       MachineBasicBlock *MBB, unsigned ValReg,
                       unsigned ShamtReg) {
  BuildMI(MBB, DL, TII->get(EVM::SLL), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
  BuildMI(MBB, DL, TII->get(EVM::SRA), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
}

bool EVMExpandPseudo::expandAtomicMinMaxOp(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    AtomicRMWInst::BinOp BinOp, bool IsMasked, int Width,
    MachineBasicBlock::iterator &NextMBBI) {
  ...
  BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW32(Ordering)), DestReg)
      .addReg(AddrReg);
  BuildMI(LoopHeadMBB, DL, TII->get(EVM::AND), Scratch2Reg)
      .addReg(DestReg)
      .addReg(MaskReg);
  BuildMI(LoopHeadMBB, DL, TII->get(EVM::ADDI), Scratch1Reg)
      .addReg(DestReg)
      .addImm(0);
  ...
  case AtomicRMWInst::Max: {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(EVM::BGE))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  case AtomicRMWInst::UMax:
    BuildMI(LoopHeadMBB, DL, TII->get(EVM::BGEU))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  BuildMI(LoopTailMBB, DL, TII->get(getSCForRMW32(Ordering)), Scratch1Reg)
      .addReg(AddrReg)
      .addReg(Scratch1Reg);
  BuildMI(LoopTailMBB, DL, TII->get(EVM::BNE))
      .addReg(Scratch1Reg)
      .addReg(EVM::X0)
      .addMBB(LoopHeadMBB);
...
bool EVMExpandPseudo::expandAtomicCmpXchg(
  ...
  if (!IsMasked) {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(EVM::BNE))
        .addReg(DestReg)
        .addReg(CmpValReg)
        .addMBB(DoneMBB);
    // .looptail:
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(EVM::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(MaskReg);
    BuildMI(LoopHeadMBB, DL, TII->get(EVM::BNE))
        .addReg(ScratchReg)
...
INITIALIZE_PASS(EVMExpandPseudo, "evm-expand-pseudo",
                EVM_EXPAND_PSEUDO_NAME, false, false)
namespace llvm {

FunctionPass *createEVMExpandPseudoPass() { return new EVMExpandPseudo(); }
