...
bool EVMAsmBackend::shouldForceRelocation(const MCAssembler &Asm,
                                            const MCFixup &Fixup,
  ...
  case EVM::fixup_evm_pcrel_lo12_s:
    ...
    const MCFixup *T = cast<EVMMCExpr>(Fixup.getValue())->getPCRelHiFixup();
    if (!T) {
  ...
  return ShouldForce || STI.getFeatureBits()[EVM::FeatureRelax] ||
         ForceRelocs;
}
...
void EVMAsmBackend::relaxInstruction(const MCInst &Inst,
  ...
  case EVM::C_BEQZ:
    ...
    Res.setOpcode(EVM::BEQ);
    Res.addOperand(Inst.getOperand(0));
    Res.addOperand(MCOperand::createReg(EVM::X0));
    Res.addOperand(Inst.getOperand(1));
    break;
  case EVM::C_BNEZ:
    // c.bnez $rs1, $imm -> bne $rs1, X0, $imm.
    Res.setOpcode(EVM::BNE);
    Res.addOperand(Inst.getOperand(0));
    Res.addOperand(MCOperand::createReg(EVM::X0));
    Res.addOperand(Inst.getOperand(1));
    break;
  ...
  case EVM::C_JAL:
    ...
    Res.setOpcode(EVM::JAL);
    Res.addOperand(MCOperand::createReg(EVM::X1));
    Res.addOperand(Inst.getOperand(0));
    break;
  }
}
...
unsigned EVMAsmBackend::getRelaxedOpcode(unsigned Op) const {
  switch (Op) {
  default:
    return Op;
  case EVM::C_BEQZ:
...
bool EVMAsmBackend::writeNopData(raw_ostream &OS, uint64_t Count) const {
  bool HasStdExtC = STI.getFeatureBits()[EVM::FeatureStdExtC];
  unsigned MinNopLen = HasStdExtC ? 2 : 4;

  if ((Count % MinNopLen) != 0)
    return false;

  // The canonical nop on EVM is addi x0, x0, 0.
...
bool EVMAsmBackend::shouldInsertExtraNopBytesForCodeAlign(
    const MCAlignFragment &AF, unsigned &Size) {
  // Calculate Nops Size only when linker relaxation enabled.
  if (!STI.getFeatureBits()[EVM::FeatureRelax])
    return false;
...
// We need to insert R_EVM_ALIGN relocation type to indicate the
// position of Nops and the total bytes of the Nops have been inserted
// when linker relaxation enabled.
// The function insert fixup_evm_align fixup which eventually will
// transfer to R_EVM_ALIGN relocation type.
bool EVMAsmBackend::shouldInsertFixupForCodeAlign(MCAssembler &Asm,
                                                    const MCAsmLayout &Layout,
                                                    MCAlignFragment &AF) {
  ...
  // Create fixup_evm_align fixup.
  MCFixup Fixup =
      MCFixup::create(0, Dummy, MCFixupKind(EVM::fixup_evm_align), SMLoc());
...
EVMAsmBackend::createObjectTargetWriter() const {
  return createEVMELFObjectWriter(OSABI, Is64Bit);
}
...
MCAsmBackend *llvm::createEVMAsmBackend(const Target &T,
  ...
  return new EVMAsmBackend(STI, OSABI, TT.isArch64Bit());
}
