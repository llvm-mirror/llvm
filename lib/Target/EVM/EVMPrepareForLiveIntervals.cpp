//===- EVMPrepareForLiveIntervals.cpp - Prepare for LiveIntervals -===//
//
...
/// EVM runs LiveIntervals in a late pass. This pass transforms code
...
#define DEBUG_TYPE "evm-prepare-for-live-intervals"

namespace {
class EVMPrepareForLiveIntervals final : public MachineFunctionPass {
public:
  static char ID; // Pass identification, replacement for typeid
  EVMPrepareForLiveIntervals() : MachineFunctionPass(ID) {}

private:
  StringRef getPassName() const override {
    return "EVM Prepare For LiveIntervals";
  }
...
char EVMPrepareForLiveIntervals::ID = 0;
INITIALIZE_PASS(EVMPrepareForLiveIntervals, DEBUG_TYPE,
                "Fix up code for LiveIntervals", false, false)

FunctionPass *llvm::createEVMPrepareForLiveIntervals() {
  return new EVMPrepareForLiveIntervals();
}
...
bool EVMPrepareForLiveIntervals::runOnMachineFunction(
    MachineFunction &MF) {
  LLVM_DEBUG({
