//===- EVMMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
void generateInstSeq(int64_t Val, bool Is64Bit, InstSeq &Res) {
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(Inst(EVM::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = (Is64Bit && Hi20) ? EVM::ADDIW : EVM::ADDI;
  ...
  Res.push_back(Inst(EVM::SLLI, ShiftAmount));
  if (Lo12)
    Res.push_back(Inst(EVM::ADDI, Lo12));
}
} // namespace EVMMatInt
} // namespace llvm
